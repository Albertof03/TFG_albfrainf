...
// Variables para almacenar capas activas
...
const getMinMax = (data, column) => {
    let values = data
        .map(row => {
            ...
            // Verificar que la fecha no esté vacía ni nula
        ...
        .filter(time => time !== null);  // Filtra valores inválidos (null)
...
// Función para cargar y almacenar datos
export const loadCSVData = async () => {
    const boundingData = await loadCSV('./datos/bounding_boxs.csv'); 
    const magnitudeData = await loadCSV('./datos/predicciones_magnitud.csv'); 
    const earthquakeCountData = await loadCSV('./datos/predicciones_terremotos.csv'); 
    ...
    BOUNDING_CSV = boundingData.map(row => ({
        ...
        coordenadas: JSON.parse(row.coordinates) // Convertir string a array
    }));
...
export const initMap = () => { 
    ...
    // Corregir aquí, usando L.control.zoom
    L.control.zoom({
        zoomInTitle: 'Acercar',
        zoomOutTitle: 'Alejar',
        position: 'topright'
...
// Función para generar el heatmap con marcadores clickeables
export const createHeatMap = (map, selectedIndex) => {
    ...
    // Si no se encuentra la fila con la fecha proporcionada, detener la ejecución
...
export const createEarthquakeCountHeatMap = (map, selectedIndex) => {
        ...
        const maxCount = Math.max(...counts, 1); // Evitar divisiones por cero
        ...
        currentHeatLayer = L.heatLayer(heatData, { 
            ...
            minOpacity: 0.6,
            gradient: {
                0.0: '#2a2a72',  // Azul oscuro (menor intensidad)
                0.1: '#009FFD',  // Azul claro
                0.3: '#00FFA2',  // Verde agua
                0.5: '#FFD500',  // Amarillo brillante
                0.8: '#FF6A00',  // Naranja fuerte
                1.0: '#D00000'   // Rojo intenso (máxima intensidad)
            }
...
// Nueva función para eliminar el heatmap
...
// Función para cargar datos de placas tectónicas
export const loadTectonicPlates = async () => {
    const platesData = await loadCSV('./datos/placas_tectonicas.csv');
...
// Función para dibujar las placas tectónicas en el mapa
...
// Nueva función para eliminar las placas tectónicas
...
// Función para agregar marcadores de terremotos
export const addEarthquakeMarkers = (map, earthquakes) => {
    clearEarthquakeMarkers(map);
    
    // Asegurarse de que earthquakes.terremotos es un arreglo
