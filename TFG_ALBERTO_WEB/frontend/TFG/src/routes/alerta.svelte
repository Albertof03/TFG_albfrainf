<script>
  ...
  import { initMap, clearEarthquakeMarkers, drawTectonicPlates, addEarthquakeMarkers, removeTectonicPlates, loadTectonicPlates } from "../../../../backend/heatmap";
  ...
  onMount(async () => {
      ...
      if (!terremotosVisible) {
          ...
          try {
              const response = await fetch(`https://tfgalbertoweb-production.up.railway.app/alerta?userId=${userId}`, {
                  method: "GET",
...
<style>
  ...
  .info-content {
    background: rgba(255, 255, 255, 0.8);  /* Fondo translúcido para destacarlo sobre el mapa */
    padding: 15px;
    border-radius: 8px;
  ...
  .menu2 {
    ...
    cursor: pointer;
    border-radius: 5px;
    z-index: 300; /* Aseguramos que esté sobre el mapa */
  }

  .menu2:hover {
    background: #3b8a99; /* Cambia el color al pasar el ratón */
  }
  ...
  main {
    ...
    align-items: center;
    justify-content: center; /* Centra el contenido si hay algo más */
  ...
  #map-container {
    ...
    top: 0;
    left: 0;
    width: 100%; /* Agrega margen */
    height:100%; /* Ajusta la altura para dejar espacio al footer */
    display: flex;
  ...
  #map {
    width:100%;
    height: 100%;
    position: relative; /* Evita que otros elementos lo tapen */
  }
  .menu-toggle {
    position: absolute;
    top: 20px;
    ...
    cursor: pointer;
    border-radius: 5px;
    z-index: 200; /* Aseguramos que esté sobre el mapa */
  }
  ...
  /* Estilo para los controles del menú */
  .controls {
    display: flex;
    flex-direction: column;
    gap: 12px;
  ...
  .title-text {
  margin-left: 10px; /* Ajusta el espacio según necesites */
}
