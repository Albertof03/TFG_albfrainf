<script>
  ...
  $: perfilEditado = { ...$user }; // Esto sincroniza cada vez que cambia el store
  ...
  async function guardarCambios() {
    try {
      ...
      const respuesta = await fetch("https://tfgalbertoweb-production.up.railway.app/perfil", {
        method: "PUT",
        headers: {
      ...
      if (respuesta.ok) {
        ...
        console.log("🆕 Datos actualizados recibidos:", datosActualizados); // Aquí lo añadimos
        user.set(datosActualizados.user); // <- Aquí accedemos al objeto 'user'
        editando = false;
      } else {
